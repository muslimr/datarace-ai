...
import { Shadow, ShadowRight } from '@assets/icons';
        ...
        <>
            <div className="relative flex justify-between mb-8">
                <div className="w-full space-y-2 z-20">
                    <h2 className="text-[32px] md:text-[2.3rem] font-semibold">{CATEGORY_LABELS[selectedCategory]} {t('competitions')}</h2>
                    <p className="text-md text-gray-700">{t('competitionDescription')}</p>
                </div>
                <div className="hidden md:flex justify-end mb-10 w-full z-20">
                ...
                {/* <div className="flex items-center absolute animate-left-svg right-0 -mt-[400px] z-0">
                    <ShadowRight />
                </div> */}
            </div>
            <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-10">
                ...
                <div className="flex md:hidden justify-center mt-10 w-full z-20">
                    <Link href={`/${lng}/races`} className="inline-flex w-auto text-center font-medium items-center px-6 py-3 text-gray-900 transition-all dark:bg-white dark:text-gray-800 rounded-xl sm:w-auto hover:bg-primaryDark hover:text-white hover:shadow-lg hover:shadow-neutral-300 hover:-translate-y-px shadow-neutral-300 dark:shadow-neutral-700 focus:shadow-none">
                        {t('allRaces')}
                        <svg className="rtl:rotate-180 w-3.5 h-3.5 ml-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                            <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M1 5h12m0 0L9 1m4 4L9 9" />
